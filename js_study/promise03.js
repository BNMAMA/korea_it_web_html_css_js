...
//         checkOut("1234-0424-1205-0910", product, (cardNumber, product) => {
...
.then((username) => {
    ...
    return addToCart ("돈까스");
})
.then((product) => {
     console.log(`${product}가 장바구니에 추가되었습니다.`);
    return checkOut ("1234-0424-1205-0910", product);
})
.then(({product, cardNumber}) => {
     console.log(`${product}에 대한 결제가 완료되었습니다.  카드번호: ${cardNumber}`);
})
.catch((error) => {
    console.log(error);
