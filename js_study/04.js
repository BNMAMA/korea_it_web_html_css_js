...
//자바 스크림트 객체/배열과 JSON문자열 간의 변환
//JSON.stringify() => 자바스크립트 객체 도는 배열을 JSON형식의 문자열로 변환
...
//요소 수정/삽입/제거: splice(시작인덱스, 제거할 개수, 추가할 요소...)
names.splice(1, 0, "김김김")
console.log(names)

//요소 찾기: find() => 주어진 테스트 함수를 만족하는 배열의 첫번 째 요소를 반환

const findFx = (n) => n === "김소현"
...
console.log(students.find((s) => s.name === "이소현"));
console.log();

//값 존재 여부: includes() - 배열에 특정 값이 포함되어 있는지 boolean으로 반환
console.log(names.includes("김소현"))
